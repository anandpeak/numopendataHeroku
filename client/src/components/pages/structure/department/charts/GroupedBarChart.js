...
class GroupedBarChart extends React.Component {
  ...
  async componentWillReceiveProps() {
    ...
    await this.setState(
      {
        numberOfGrade: this.props.numberOfGrade,
        gradeChartData: [
          {
            key: 'F',
            values: [
  ...
  removePreviusChar() {
    let chart = document.getElementsByClassName('GBarChart')[0];
  ...
  renderNormalDist = () => {
    const departGrade = this.props.numberOfGrade;
  ...
  render() {
    return (
      <React.Fragment>
        <div className="depGPChartContainer">
          <div className="titleOfDepGPChart">
            Салбар сургууль болон тэнхимийн дүнгийн мэдээллийг харьцуулсан
            статистк график. (2018 оны намрын улирал)
            <h5>Зөвхөн A, B, C, D, F үнэлгээнүүдийг тооцсон болно.</h5>
          </div>
          <div className="GBarChart"></div>
        </div>
      </React.Fragment>
    );
  }
